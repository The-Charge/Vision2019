...
class DeepSpaceVision:
    ...
    def __init__(self):
        ...
        self.dist_coeff = [2.0088620543456895e-01,
                           8.1182994043455903e-01,
                          -7.8605490125169836e-03,
                          -2.7920374868431819e-03,
                          -7.9665818965557920e+00 ]
    ...
    def findTargets(self, img):
        ...
        for i in range(len(contourInfo)-1):
            ...
            if angle_l > 0 and angle_r < 0 and cx_l < cx_r:
                ...
                # Find the image points of the target
                img_points = []
                ...
                # Format the image points
                img_points = np.float32(img_points)
                
                # Run the solvePnP algorithm to calculate transformational matrices
                ...
                if ret:
                    ...
                    if self.EDIT_IMAGE:
                        ...
                        cv2.rectangle(img, rect, (140, 145, 160), 2)
                    
                        # Find the center of the two targets
                        tar_x, tar_y = (int(rx+rw/2), int(ry+rh/2))
                        
                        # Draw a circle in the middle of both contours
                        cv2.circle(img, (tar_x, tar_y), 2, (160, 20, 20), 3)

                        # Write text information of the calculated distances and angles
                        cv2.putText(img, "{:.4}in".format(distance), (c_x+text_x, c_y+text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255,255,255))
                        cv2.putText(img, "{:.4}dg yaw".format(yaw), (c_x+text_x, c_y+text_y + 1*text_gap), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255,255,255))
                        cv2.putText(img, "{:.4}dg skew".format(rotation), (c_x+text_x, c_y+text_y + 2*text_gap), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255,255,255))
